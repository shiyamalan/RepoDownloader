...
branch=$3
	...
	logs=`svn log $url\$bfqn | grep "^r" | tail --lines=1`
	#echo "Logs are "$logs
	...
	#`svn info -r{"2007-04-19 10:11:07"} "https://subversion.cambio.se/PC/Standard/ApplicationLoggerModuleClient/branches/R8.1_ApplicationLoggerModuleClient_Dev/" | grep Revision | cut -d' ' -f2`
	...
	#echo "REVISIIOOOOON------------------ "$rev
	#revision=$(echo $rev | tr "|" "\n")
	#`svn log --revision {2007-01-01}:{2007-11-24} --limit 1` 
	#`svn log --stop-on-copy $url\$bfqn | gawk '{print $1}' | grep "^r" | tail --lines=1`
	#echo "--------------------------------------- Revision Befor Parsing "${revision[0]}
    #rev=${revision:1}
	#echo "--------------------------------------- Revision After Parsing "$rev
...
git svn fetch -r$minrev:HEAD #fetch the repo from minrevision id to latest updates(HEAD)